 ...
 * Class		: NpbRightContentController
...
namespace Splg.Areas.Npb.Controllers
{
    ...
    /// </summary>
    public class NpbRightContentController : Controller
    {
        ...
        public ActionResult EditPointInMobile(string sportID, int? gameID, string expectTargetID, int betSelectID, string memberID, int teamID, string reloadurl, string reloadarea)
        {
            if(gameID != null)
            {
                ...
                query = (from et in com.ExpectTarget
                         ...
                         where et.ExpectTargetID == lgExtTarget && et.UniqueID == gameID && ep.CreatedAccountID == memberID && ep.BetSelectID == betSelectID
                             && ep.UniqueID == teamID
                         select new ExpectationInfoModel
                         {
                             ...
                             Odds = (from od in com.OddsInfo
                                     ...
                                     where ep.BetSelectID == bsm.BetSelectID
                                         && et.SportsID == spID && et.SportsID == od.SportID && od.ExpectTargetID == et.ExpectTargetID
                ...
                return PartialView("_NpbPointEdit", query);
            }
            else
            {
                return PartialView("_NpbPointEdit");
            }
        ...
        public IEnumerable<NewsInfoViewModel> GetNpbNews(string type, int? teamID)
        {
            ...
            if (!string.IsNullOrEmpty(type))
            {
                ...
                else if (type == Constants.NPBTEAMINFOTOP || type == Constants.NPBTEAMINFOPITCHER || type == Constants.NPBGAMEINFO || type == Constants.NPBTEAMINFOBATCHER)
                {
        ...
        {
            var query = (from hittingStats in npb.HittingStats
                         join hittingStatsH in npb.HittingStatsHeader on hittingStats.HittingStatsHeaderId equals hittingStatsH.HittingStatsHeaderId
                         join playerInfoMST in npb.PlayerInfoMST on hittingStats.PlayerCD equals playerInfoMST.PlayerCD
                         join playerHeader in npb.PlayerInfoMSTHeader on new { k1 = playerInfoMST.PlayerInfoMSTHeaderId, k2 = hittingStats.TeamCD } equals new { k1 = playerHeader.PlayerInfoMSTHeaderId, k2 = playerHeader.TeamCD }
                         where hittingStats.PlayerCD == playerID
                         where hittingStats.TeamCD == teamID
                         where hittingStatsH.GameAssortment != Constants.OFFICIALSTATS_GAMEASSORTMENT_EXCHANGE_GAME
                         select hittingStats
                         ).FirstOrDefault(); 
            return query;
        }
        ...
        {
            int nxtDate = 0;
            nxtDate = npb.GameInfoSS.Where(m => m.GameDate > gameDate).Min(m => m.GameDate);
