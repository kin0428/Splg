...
$(document).ready(function () {
    var urlCommon = "/Mlb/MlbTop/ShowGameInfo";
    var dt = new Date();
    ...
    $('.j_rank a:not(.active)').live('click', function (event) {
        ...
        $("#Page2").attr('data-startdate', firstDateWeek);
        $("#Page2").attr('data-enddate', lastDateWeek);
    ...
    */
    function getDayOfWeek(year, month, week, day) {
        ...
        if (firstDay == 1) {
            ...
            var day1 = (7 - firstDay + 1) + 7 * (week - 1) + 1;
        }

        result = new Date(year + "/" + month + "/" + day1);
     
        if (day > 1) {
            result = new Date(result.getTime() + (day - 1) * 24 * 60 * 60 * 1000);
        }
